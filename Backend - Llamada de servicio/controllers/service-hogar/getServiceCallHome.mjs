...
const getServiceCallHome = async (req, res) => {
  const cookies = req.header("Authorization");
  try {
    const getOrigins = await clienteAxios.get(
      `/b1s/v1/ServiceCallOrigins?$filter=startswith(Description,'ACT-H')`,
      {
        headers: {
          Cookie: `${cookies}`,
    ...
    const getProblemTypes = await clienteAxios.get(
      `/b1s/v1/ServiceCallProblemTypes?$filter=contains(Description, '-H') and Active eq 'tYES'`,
      {
        headers: {
          Cookie: `${cookies}`,
          Prefer: "odata.maxpagesize=0",
    ...
    const getProblemSubTypes = await clienteAxios.get(
      `/b1s/v1/ServiceCallProblemSubTypes?$filter=contains(Description, '-H') and Active eq 'tYES'`,
      {
        headers: {
          Cookie: `${cookies}`,
          Prefer: "odata.maxpagesize=0",
...
export default getServiceCallHome